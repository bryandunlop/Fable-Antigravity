      ...
      <div className="flex gap-2 flex-wrap mb-2">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setSub(t.key)}
            className={`text-[14px] font-semibold px-4 py-2 min-h-[44px] rounded-full border transition-colors ${sub === t.key ? 'bg-secondary text-secondary-foreground border-secondary' : 'bg-card text-muted-foreground border-border hover:border-muted-foreground/40'}`}>
            {t.label}
          </button>
        ))}
      </div>
