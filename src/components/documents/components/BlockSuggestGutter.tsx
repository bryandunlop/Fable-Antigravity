...
export function BlockSuggestGutter({
  openCount, canSeePins, active, onSuggest, onOpenThread, children,
}: {
  openCount: number;
  canSeePins: boolean;
  active: boolean;
  ...
  return (
    <div className="absolute right-0 top-1">
      ...
      {active && children && (
        <div className="absolute right-0 top-8 z-20 w-80 rounded-lg border border-sky-200 bg-card p-3 shadow-md dark:border-sky-900">
          {children}
        </div>
      )}
    </div>
