...
function storedState(overrides: Partial<DocumentsState> = {}): DocumentsState {
  return {
    docs: [userDoc()],
    revisions: [userRev()],
    acknowledgments: [
...
describe('loadInitialState (C5 — migrate, don’t wipe)', () => {
  ...
  it('a DATA_VERSION bump stamps the new version and persists the migrated state', () => {
    localStorage.setItem(VERSION_KEY, '2020-01-01-v0');
    localStorage.setItem(STORAGE_KEY, JSON.stringify(storedState()));
