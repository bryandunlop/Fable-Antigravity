...
import { DEFAULT_TEMPLATE, recipientsFor, renderEmail, draftEmail, sendEmail, autoSendIfDue, emailState, editDraftBlock, type PassengerPref } from './briefingEmail';
...
describe('who gets it', () => {
  it('honours every / first-trip-only / never; unknown people are sent to', () => {
    ...
    expect(recipientsFor(sheet, PREFS)).toEqual(['S. Reyes']);
    expect(recipientsFor(sheet, [])).toEqual(['A. Reyes', 'S. Reyes', 'K. Tanaka']);
  });
});
...
describe('drafted at freeze, sent by scheduling, or by the dead-man switch', () => {
  it('drafts with the recipients and a 12 h timer; scheduling sends; the EA cannot', () => {
    let t = draftEmail(trip(), buildSheet(trip(), CTX, T, SCHED), DEFAULT_TEMPLATE, PREFS, WX, SCHED, T);
  ...
  it('the dead-man switch fires at the timer and says unreviewed; not a minute before', () => {
    const t = draftEmail(trip(), buildSheet(trip(), CTX, T, SCHED), DEFAULT_TEMPLATE, PREFS, WX, SCHED, T);
