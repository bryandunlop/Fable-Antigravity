...
export default function AdminMel() {
  ...
  const approve = (id: string) => {
    const m = state.melItems.find(x => x.id === id)!;
    dispatch({ type: 'EDIT_MEL_ITEM', payload: { ...m, approvalState: 'APPROVED' } });
    dispatch({ type: 'ADD_AUDIT', payload: { id: newId('aud'), actorOid: user.oid, action: 'MEL_APPROVED', entityType: 'MelItem', entityId: id, atUtc: new Date().toISOString(), summary: `Approved MEL ${m.subItemNumber} (${m.aircraftType})` } });
    toast.success(`Approved MEL ${m.subItemNumber}.`);
  };

  return (
    <TechLogShell
      title="Admin · MEL Management"
      subtitle="D195 content by type, revision, and approval state — the source the deferral workflow reads."
      actions={
          ...
          <Card key={m.id}>
            <CardContent className="flex flex-col gap-2 p-3 md:flex-row md:items-center md:justify-between">
              ...
              {m.approvalState !== 'APPROVED' && canEdit && (
                <Button size="sm" onClick={() => approve(m.id)}><BadgeCheck className="mr-1.5 h-4 w-4" /> Approve</Button>
              )}
            </CardContent>
          </Card>
        ))}
