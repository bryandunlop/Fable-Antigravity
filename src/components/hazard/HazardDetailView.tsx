...
export default function HazardDetailView({ userRole = 'safety' }: { userRole?: string }) {
  ...
  // Data States
  const [riskSeverity, setRiskSeverity] = useState(0);
                  ...
                  <CardContent className="pt-4 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="p-4 border rounded-lg bg-white relative shadow-sm">
                        {riskSeverity > 0 && riskLikelihood > 0 && <CheckCircle className="w-5 h-5 text-green-500 absolute top-4 right-4" />}
                        <h4 className="font-bold mb-1 flex items-center gap-2">Risk Assessment</h4>
                        {riskSeverity > 0 && riskLikelihood > 0 ? (
                           <p className="text-sm text-slate-600 font-medium">Risk Score: <span className={`${getScoreColor(riskSeverity + riskLikelihood)} px-2 py-0.5 rounded text-black inline-block ml-1`}>{riskSeverity + riskLikelihood}</span> (S: {riskSeverity}, L: {riskLikelihood})</p>
                        ) : (
                        ...
                        <Button variant="outline" size="sm" className="mt-3 w-full" onClick={() => setShowRiskWizard(true)}>
                          {riskSeverity > 0 ? 'Edit Matrix Assessment' : 'Start Risk Assessment'}
                        </Button>
                      </div>
                  ...
                  <CardContent className="pt-4">
                    ...
                    {currentStage === WORKFLOW_STAGES.ASSIGN_MITIGATION ? (
                      <div className="space-y-6">
                        ...
                        <Button 
                          className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold h-12 shadow-lg shadow-indigo-100 flex items-center justify-center gap-2 text-base transition-all active:scale-[0.98]"
                  ...
                  <CardContent className="pt-4 space-y-4">
                    {currentStage === WORKFLOW_STAGES.SM_MITIGATION_REVIEW ? (
                      <div className="space-y-6">
                        ...
                        <Button 
                          className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold h-12 shadow-lg shadow-orange-100 flex items-center justify-center gap-2 text-base transition-all active:scale-[0.98]"
                  ...
                  <CardContent className="pt-4 space-y-4">
                    {currentStage === WORKFLOW_STAGES.SM_POST_MANAGER ? (
                      <div className="space-y-6">
                        ...
                        <Button 
                          className="w-full bg-red-600 hover:bg-red-700 text-white font-bold h-12 shadow-lg shadow-red-100 flex items-center justify-center gap-2 text-base transition-all active:scale-[0.98]"
                ...
                {isSafetyManager && (
                  <>
                    {currentStage === WORKFLOW_STAGES.SUBMITTED && (
                       <Button className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-100 px-6" onClick={advanceStage}>Start Investigation <ArrowRight className="w-4 h-4 ml-2" /></Button>
                    )}
                    {currentStage === WORKFLOW_STAGES.SM_INVESTIGATION && (
                       <Button className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-100 px-6" onClick={advanceStage}>Complete Investigation & Route <ArrowRight className="w-4 h-4 ml-2" /></Button>
                    )}
                    {currentStage === WORKFLOW_STAGES.ASSIGN_MITIGATION && (
                       <Button 
                    ...
                    {currentStage === WORKFLOW_STAGES.MITIGATION_DEVELOPMENT && (
                       <Button className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-100 px-6" onClick={advanceStage}>Review Mitigations <ArrowRight className="w-4 h-4 ml-2" /></Button>
                    )}
                    {currentStage === WORKFLOW_STAGES.SM_MITIGATION_REVIEW && (
                       <Button 
                         className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-100 px-6" 
                    ...
                    {currentStage === WORKFLOW_STAGES.SM_POST_EXEC && (
                       <div className="flex gap-2">
                         ...
                         <Button className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-100 px-6" onClick={advanceStage}>Prepare Final Report <ArrowRight className="w-4 h-4 ml-2" /></Button>
                       </div>
                    )}
         ...
         <DialogContent className="max-w-[90vw] w-[1000px] h-[90vh] flex flex-col p-6">
                  ...
                  {severities.map((severity, sIdx) => (
                     <React.Fragment key={severity.score}>
                        ...
                        {likelihoods.map(likelihood => {
                           ...
                           const isDimmed = (riskSeverity > 0 || riskLikelihood > 0) && !isSelected && !(riskSeverity === severity.score && riskLikelihood === 0) && !(riskLikelihood === likelihood.score && riskSeverity === 0);
                           const score = severity.score + likelihood.score;
                           const cellClass = `
                           ...
                           return (
                              ...
                              >
                                 <div className={`text-2xl font-black ${score > 3 && score < 7 ? 'text-slate-900' : ''}`}>
                                    {score}
                                 </div>
                                 {isSelected && (
            ...
            <div className="shrink-0 mt-6 grid grid-cols-2 gap-6 pb-2">
               ...
               <div className="text-right">
                  ...
                  >
                     Confirm Risk Score ({riskSeverity + riskLikelihood || '?'})
                  </Button>
               </div>
            </div>
         </DialogContent>
      ...
      <Dialog open={isPreviewingReport} onOpenChange={setIsPreviewingReport}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter className="bg-slate-50/50 p-6 -m-6 mt-4 border-t gap-2 sm:justify-end">
            ...
            <Button className="bg-green-600 hover:bg-green-700 text-white px-8 h-10 shadow-md shadow-green-100 font-bold" onClick={() => {
           ...
           <div className="border-t pt-6 flex items-center justify-between">
              ...
              <DialogFooter className="gap-3 sm:justify-end">
                ...
                <Button 
                  ...
                  className="bg-green-600 hover:bg-green-700 text-white px-8 font-bold shadow-lg shadow-green-100 min-w-[200px]"
                  onClick={() => {
