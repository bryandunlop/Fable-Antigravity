...
**F1. SAFA ramp checks — crew visibility. ✅ BUILT 2026-07-24.** (Requested 2026-07-22.) SAFA (Safety Assessment of Foreign Aircraft) ramp-check readiness, surfaced to crew before international legs. Built as a **hybrid, informational-only** feature:
- **Two layers.** The check *definition* (what an inspector reviews, by SAFA area A–E) is **compliance-owned myGFO reference data** (`state.safaCheckItems`, seed `mockData/safa.ts`), curated by a **Reg & Comp** role under **four-eyes** (`PROPOSE_CHANGE`/`DECIDE_APPROVAL`, kind `SAFA_CHECKLIST_EDIT`; `engine/approvals.applyApproval`). Per-aircraft **status/expiry** is a **read-only CAMP overlay** (`integration/campClient.campSafaStatus`, `useIntegration.readSafaStatus`), joined by item `code` in `engine/safa.deriveSafaReadiness`.
- **Role.** The existing `reg-comp` login role now resolves to a seeded compliance persona carrying `Personnel.regComplianceAuthorized` (two seeded in `fleet.ts`); `engine/safa.canEditSafaChecklist` gates editing. Approvals are decidable by a *different* Reg & Comp user or any maintenance user (`PendingApprovalsPanel`); self-approval is reducer-blocked.
- **Surfaces.** New `pages/Safa.tsx` (route `/tech-log/safa`; nav in both pilot and maintenance-Airworthiness), a **SAFA ramp-check readiness** block in the Flight Briefing (`BriefingPanel`, + printed), and an advisory pilot notification on `ACTION` items. Advisory only — never touches serviceability/dispatch.
- **Still an Open Question:** the real CAMP read function for SAFA status is undocumented — the mock is flagged (`CAMP_SAFA_OPEN_QUESTION`, logged as `GetSafaReadiness (TBC fn)`); confirm with CAMP under sandbox before wiring the live read (no invented endpoint). Tests in `engine/safa.test.ts`; `DATA_VERSION` → `2026-07-24-v14`.

**F2. SAFA auditor report — format & content. 🚩 DISCUSS WITH REG & COMP (do not build yet).** (Flagged 2026-07-24 by Bryan.) A printable, auditor-facing "SAFA Ramp-Check Readiness" report was **prototyped** (as a standalone document with a provenance/controls panel, fleet summary, and per-aircraft readiness tables; demonstration/sandbox data). Bryan wants to settle the format **with Regulatory & Compliance before productionizing** — Reg & Comp owns the SAFA definition, so they should own what the audit deliverable looks like. Open points to raise:
- **Granularity — leaning per-aircraft.** Bryan's current thinking is **one report per tail** (a document per aircraft for the auditor) rather than a fleet roll-up. Confirm with Reg & Comp.
- **Content an auditor needs:** which fields; an **attestation / signer block** (name, title, date, signature line); governing revision/effective date; evidence references per item.
- **Where it lives:** an in-app **"View / Print" export** off `pages/Safa.tsx` (mirroring the existing CRS/briefing print pattern via `util/printRecord.ts`) and/or an on-demand/scheduled export.
- **Live-data labeling:** drop the "Demonstration — not a certified record" stamp only once wired to real CAMP; keep the **point-in-time** capture semantics. Blocked on F1's CAMP-read Open Question.
