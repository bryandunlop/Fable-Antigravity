...
3. **Time bands** (by earliest-leg departure vs now): `This week` (≤7d) · `Next 2 weeks` (8–14d) · `Later this month` (15–30d) · `Next month` (31–60d, and anything beyond). Soonest-first within each.
...
- **Trip card:** a readiness dot (green ready / amber needs-attention / muted upcoming, from `PilotReadiness`), tail · route · departure date, an INTL/DASSP badge, a **"needs prep"** flag when `tripNeedsPrep` is true, and a chevron. Tapping calls the existing `onOpen(trip)` → Slice-2 `FlightHub`.
...
- `groupTripsByHorizon`: an `in_progress` trip lands in `inProgress` regardless of its departure; band boundaries at exactly 7 / 14 / 30 / 60 days; a trip beyond 60d lands in `nextMonth`; soonest-first ordering within a band; empty bands omitted.
...
- **A1** Band boundaries 7 / 14 / 30 / 60 days (named constants).
